...
class Facebook_Feed_Post_Types extends Facebook_Feed {
	...
	public function feed_post_types( $set_zero, $fb_type, $post_data, $fb_shortcode, $response_post_array, $single_event_array_response = null ) {
		...
		$fb_final_story               = '';
		$first_dir                    = '';
		$fb_picture                   = $post_data->picture ?? '';
		$fb_album_link                = $post_data->link ?? '';
		$fb_link                      = $post_data->attachments->data[0]->url ?? $fb_album_link;
		$fb_name                      = $post_data->name ?? '';
		$fb_caption                   = $post_data->attachments->data[0]->caption ?? '';
		$fb_description               = $post_data->attachments->data[0]->description ?? '';
		$fb_link_event_name           = $post_data->to->data[0]->name ?? '';
		$fb_story                     = $post_data->story ?? '';
		$fb_icon                      = $post_data->icon ?? '';
		$fb_by                        = $post_data->properties->text ?? '';
		$fb_bylink                    = $post_data->properties->href ?? '';
		$fb_post_share_count          = $post_data->shares->count ?? '';
		$fb_post_like_count_array     = $post_data->likes->data ?? '';
		$fb_post_comments_count_array = $post_data->comments->data ?? '';
		$fb_post_object_id            = $post_data->attachments->data[0]->id ?? '';
		$fb_album_photo_count         = $post_data->count ?? '';
		$fb_album_cover               = $post_data->photos->data[0]->images[0]->source ?? '';
		$fb_album_picture             = $post_data->source ?? '';
		$fb_places_name               = $post_data->place->name ?? '';
		$fb_places_id                 = $post_data->place->id ?? '';

		$fb_attachments_title         = $post_data->attachments->data[0]->title ?? '';
		$fb_attachments               = $post_data->attachments ?? '';
		$fb_picture_job               = $post_data->attachments->data[0]->media->image->src ?? '';
		// youtube and vimeo embed url.
		$fb_video_embed               = $post_data->attachments->data[0]->media->source ?? '';

		$fb_post_from_id              = $post_data->from->id ?? '';
				...
				if ( isset( $fb_shortcode['popup'] ) && 'yes' === $fb_shortcode['popup'] ) {
					...
					if ( 'albums' === $fb_shortcode['type'] ) {
						...
						foreach ( $post_data->photos->data as $key => $fb_album_additional_pic ) {
							...
							// Albums Photo Count.
							$fb_name ? $this->fts_facebook_post_desc( $fb_name, $fb_shortcode, $fb_type, null, $fb_by ) : '';
							$view_additional_album_photos = '24' == $key ? '. <a href="' . $fb_album_link . '" target="_blank" rel="noreferrer">' . esc_html__( 'View more for this Album', 'feed-them-social' ) . '</a>' : '';
							...
							$fb_album_single_picture_id = isset( $fb_album_additional_pic->id ) ? $fb_album_additional_pic->id : '';
							$fb_single_image_link = 'https://www.facebook.com/'. $fb_album_single_picture_id . '';
							$this->fts_facebook_post_see_more( $fb_single_image_link, $lcs_array, $fb_type, $fb_post_id, $fb_shortcode, $fb_post_user_id, $fb_post_single_id, $single_event_id, $post_data );
