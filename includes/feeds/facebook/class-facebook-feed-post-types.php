...
class Facebook_Feed_Post_Types {
	...
	public function facebook_post_photo( $fb_link, $saved_feed_options, $photo_from, $photo_source ) {
		if ( $saved_feed_options['facebook_page_feed_type'] === 'album_photos' || $saved_feed_options['facebook_page_feed_type'] === 'albums' ) {
			...
			if ( $this->feed_functions->is_extension_active( 'feed_them_social_premium' ) && $saved_feed_options_popup === 'yes' && $fb_link !== 'javascript:;' ) {
			...
			if ( isset( $lcs_array['comments_thread']->data ) && ! empty( $lcs_array['comments_thread']->data ) && 'yes' !== $hide_comments_popup || isset( $lcs_array['comments_thread']->data ) && ! empty( $lcs_array['comments_thread']->data ) && empty( $hide_comments_popup ) ) {
				...
				foreach ( $lcs_array['comments_thread']->data as $comment ) {
					if(!empty($comment->message)) {
						...
						// User Profile Img.
                        $comment_profile_url_check = isset( $comment->from->id ) ? 'https://graph.facebook.com/'.$comment->from->id.'?fields=picture&access_token='. $this->access_options->decryptAccessToken($saved_feed_options['fts_facebook_custom_api_token']) : 'WTF';
