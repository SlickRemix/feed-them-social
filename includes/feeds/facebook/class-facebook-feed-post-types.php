...
class Facebook_Feed_Post_Types {
	...
	public function feed_post_types( $set_zero, $facebook_post_type, $facebook_post, $saved_feed_options, $response_post_array, $single_event_array_response = null ) {
		...
		if ( isset( $scrollhorz_or_carousel, $saved_feed_options['slider_spacing'] ) && ! empty( $saved_feed_options['slider_spacing'] ) && 'carousel' === $saved_feed_options['scrollhorz_or_carousel']) {
		...
		switch ( $facebook_post_type ) {
			...
			case 'video_inline' :
				...
				if ( isset( $saved_feed_options['facebook_grid_column_width'] ) && 'yes' === $saved_feed_options['facebook_grid_column_width'] ) {
					echo 'style="width:' . esc_attr( $saved_feed_options['facebook_grid_column_width'] ) . '!important; margin:' . esc_attr( $saved_feed_options['facebook_grid_column_width'] ) . '!important"';
				}
				echo '>';
			...
			case 'photo':
				...
				if ( 'album_photos' === $saved_feed_options['facebook_page_feed_type'] || 'albums' === $saved_feed_options['facebook_page_feed_type'] ) {
					...
					if ( isset( $saved_feed_options['scrollhorz_or_carousel']) && 'scrollhorz' === $saved_feed_options['scrollhorz_or_carousel']) {
						echo 'style="max-width:' . esc_attr( $saved_feed_options['facebook_image_width'] ) . ';height:100%;  margin:' . esc_attr( $saved_feed_options['facebook_space_between_photos'] ) . '!important"';
					} else {
						echo 'style="width:' . esc_attr( $saved_feed_options['facebook_image_width'] ) . ' !important; height:' . esc_attr( $saved_feed_options['facebook_image_height'] ) . '!important; margin:' . esc_attr( $saved_feed_options['facebook_space_between_photos'] ) . '!important"';
					}
				}
				if ( isset( $saved_feed_options['facebook_grid_column_width'] ) && 'yes' === $saved_feed_options['facebook_grid_column_width'] ) {
					echo 'style="width:' . esc_attr( $saved_feed_options['facebook_grid_column_width'] ) . '!important; margin:' . esc_attr( $saved_feed_options['facebook_grid_column_width'] ) . '!important"';
			...
			if ( $facebook_message && 'top' !== $show_media ) {
				...
				if ( isset( $saved_feed_options['facebook_popup']  ) && 'yes' === $saved_feed_options['facebook_popup']  ) {
					...
					elseif (
						...
						if ( 'yes' !== $saved_feed_options['facebook_play_btn']  ) {
...
>' . esc_html( 'Get Directions', 'feed-them-social' ) . '</a>';
			...
			case 'share':
				...
				$saved_feed_options['facebook_page_word_count']= isset( $saved_feed_options['facebook_page_word_count']) ? $saved_feed_options['facebook_page_word_count']: '';
				// Description Wrap.
				echo '<div class="fts-jal-fb-description-wrap">';
				// Output Link Name.
				$this->fts_facebook_post_name( $facebook_post_link, $facebook_post_name, $facebook_post_type );
			...
			case 'photo':
				...
				if ( isset( $saved_feed_options['facebook_video_album'] ) && 'yes' === $saved_feed_options['facebook_video_album'] ) {
					if ( isset( $saved_feed_options['facebook_play_btn']  ) && 'yes' === $saved_feed_options['facebook_play_btn']  ) {
						...
						echo '<a href="' . esc_url( $facebook_post_embed_html ) . '"  data-poster="" id="fts-view-vid1-' . esc_attr( $fts_dynamic_vid_name_string ) . '" title="' . esc_html( $facebook_post_description ) . '" class="fts-jal-fb-vid-html5video ' . esc_attr( $fts_view_fb_videos_btn . ' fb-video-popup-' . $fts_dynamic_vid_name_string . ' ' . $facebook_play_btn_visible ) . ' fts-slicker-backg" style="height:' . esc_attr( $saved_feed_options['facebook_play_btn_size'] ) . ' !important; width:' . esc_attr( $saved_feed_options['facebook_play_btn_size'] ) . '; line-height: ' . esc_attr( $saved_feed_options['facebook_play_btn_size'] ) . '; font-size:' . esc_attr( $saved_feed_options['facebook_play_btn_size'] ) . '"><span class="fts-fb-video-icon" style="height:' . esc_attr( $saved_feed_options['facebook_play_btn_size'] ) . '; width:' . esc_attr( $saved_feed_options['facebook_play_btn_size'] ) . '; line-height:' . esc_attr( $saved_feed_options['facebook_play_btn_size'] ) . '; font-size:' . esc_attr( $saved_feed_options['facebook_play_btn_size'] ) . '"></span></a>';
		...
		if ( isset( $saved_feed_options['facebook_show_media']) && 'top' === $show_media ) {
			...
			if ( isset( $saved_feed_options['facebook_show_social_icon'] ) && 'right' === $saved_feed_options['facebook_show_social_icon'] ) {
